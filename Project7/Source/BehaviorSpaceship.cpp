...
{
	BehaviorPtr newBehaviorObject = (BehaviorPtr)calloc(1, sizeof(Behavior));

	if (newBehaviorObject != NULL)
	{
		newBehaviorObject->stateCurr = (int)behaviorStates::cSpaceshipInvalid;
		newBehaviorObject->stateNext = (int)behaviorStates::cSpaceshipInvalid;
...
void BehaviorSpaceshipInit(BehaviorPtr behavior)
{
	...
	if(behavior->stateCurr == (int)behaviorStates::cSpaceshipIdle)
	{
	...
	if(behavior->stateCurr == (int)behaviorStates::cSpaceshipDead)
	{
...
void BehaviorSpaceshipUpdate(BehaviorPtr behavior, float dt)
{
	if(behavior != NULL)
	{
		...
		{
		case (int)behaviorStates::cSpaceshipIdle:
			BehaviorSpaceshipUpdateRotation(behavior, dt);
			BehaviorSpaceshipUpdateWeapon(behavior, dt);
			if (AEInputCheckCurr(VK_UP))
			{
				behavior->stateNext = (int)behaviorStates::cSpaceshipThrust;
			}
			break;

		case (int)behaviorStates::cSpaceshipThrust:
			BehaviorSpaceshipUpdateRotation(behavior, dt);
			BehaviorSpaceshipUpdateVelocity(behavior, dt);
			BehaviorSpaceshipUpdateWeapon(behavior, dt);
			if (!AEInputCheckTriggered(VK_UP))
			{
				behavior->stateNext = (int)behaviorStates::cSpaceshipIdle;
			}
			break;
		}
...
static void BehaviorSpaceshipCollisionHandler(GameObjectPtr gameObject, GameObjectPtr other)
{
	if(gameObject != NULL && other != NULL)
	{
		...
		if(strncmp(name, "Asteroid", _countof("Asteroid")) == 0)
		{
			...
			behavior->stateNext = (int)behaviorStates::cSpaceshipDead;
		}
	}
}