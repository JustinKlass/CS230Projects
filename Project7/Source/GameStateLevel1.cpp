...
static enum MonkeyStates monkeyState = MonkeyStates::MonkeyInvalid;
...
void GameStateLevel1Init()
{
	...
	{
		monkeyState = MonkeyStates::MonkeyInvalid;
		GameStateLevel1SetMonkeyState(newMonkeyObj, MonkeyStates::MonkeyIdle);
	...
	if(newPhysics != NULL && newTransform != NULL)
	{
		...
		if(&currVelocity != NULL)
		{
			if(AEInputCheckCurr(VK_LEFT))
			{
				...
				// TO FLIP THE MONKEY
				if(monkeyState != MonkeyStates::MonkeyJump)
				{
					GameStateLevel1SetMonkeyState(gameObject, MonkeyStates::MonkeyWalk);
				}
			}
			...
			else
			{
				...
				if (monkeyState != MonkeyStates::MonkeyJump)
				{
					GameStateLevel1SetMonkeyState(gameObject, MonkeyStates::MonkeyIdle);
				}
			}
			...
			if(AEInputCheckTriggered(VK_UP))
			{
				...
				GameStateLevel1SetMonkeyState(gameObject, MonkeyStates::MonkeyJump);
