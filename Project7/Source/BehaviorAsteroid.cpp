...
{
	BehaviorAsteroidPtr newAsteroid = (BehaviorAsteroidPtr)calloc(1, sizeof(BehaviorAsteroid));
	...
	if (newAsteroid != NULL)
	{
		...
		newAsteroid->size = AsteroidSize::cAsteroidLarge;
...
void BehaviorAsteroidInit(BehaviorPtr behavior)
{
	...
	if(asteroidPtr->base.stateCurr == (int)AsteroidStates::cAsteroidIdle)
	{
		asteroidPtr->origin = (AsteroidOrigin)RandomRange(0, 3);
...
static void BehaviorAsteroidSetPosition(BehaviorAsteroidPtr asteroidPtr)
{
	if(asteroidPtr->size == AsteroidSize::cAsteroidLarge)
	{
		...
		if(asteroidPtr->origin == AsteroidOrigin::cAsteroidOriginTlc)
		{
			Vector2DSet(&newPosition, minX, maxY);
			TransformSetTranslation(transform, &newPosition);
			
		}
		else if(asteroidPtr->origin == AsteroidOrigin::cAsteroidOriginBlc)
		{
			Vector2DSet(&newPosition, minX, minY);
			TransformSetTranslation(transform, &newPosition);
		}
		else if(asteroidPtr->origin == AsteroidOrigin::cAsteroidOriginTrc)
		{
			Vector2DSet(&newPosition, maxX, maxY);
			TransformSetTranslation(transform, &newPosition);
		}
		else if(asteroidPtr->origin == AsteroidOrigin::cAsteroidOriginBrc)
		{
...
static void BehaviorAsteroidSetVelocity(BehaviorAsteroidPtr asteroidPtr)
{
	...
	if(asteroidPtr->size == AsteroidSize::cAsteroidMedium || asteroidPtr->size == AsteroidSize::cAsteroidSmall)
	{
