...
{
	BehaviorHudTextPtr newHudText = (BehaviorHudTextPtr)calloc(1, sizeof(BehaviorHudText));
	...
	if (newHudText != NULL)
	{
		...
		newHudText->base.stateCurr = (int)HudTextStates::cHudTextInvalid;
		newHudText->base.stateNext = (int)HudTextStates::cHudTextInvalid;
		newHudText->scoreSystemId = ScoreSystemId::SsiInvalid;
...
void BehaviorHudTextRead(BehaviorPtr behavior, Stream stream)
{
	if(behavior != NULL && stream != NULL)
	{
		...
		behaviorHud->scoreSystemId = (ScoreSystemId)StreamReadInt(stream);
	}
}
...
static void BehaviorHudTextUpdateText(BehaviorHudTextPtr behavior)
{
	if(behavior != NULL)
	{
		if(behavior->scoreSystemId != ScoreSystemId::SsiInvalid)
		{
