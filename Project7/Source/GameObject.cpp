...
{
	GameObjectPtr newGameObject = (GameObjectPtr)calloc(1, sizeof(GameObject));
...
void GameObjectRead(GameObjectPtr gameObject, Stream stream)
{
	...
	while(true)
	{
		...
		else if(strncmp(streamVar, "BehaviorHudText", _countof("BehaviorHudText")) == 0)
		{
			...
			BehaviorHudTextRead(behaviorHudText, stream);
			GameObjectAddBehavior(gameObject, behaviorHudText);
		}
		else if(strncmp(streamVar, "ColliderCircle", _countof("ColliderCircle")) == 0)
		{
			ColliderPtr colliderObj = ColliderCircleCreate();
			ColliderCircleRead(colliderObj, stream);
			GameObjectAddCollider(gameObject, colliderObj);
		}
		else if (strncmp(streamVar, "ColliderLine", _countof("ColliderLine")) == 0)
		{
			ColliderPtr colliderObj = ColliderLineCreate();
			ColliderLineRead(colliderObj, stream);
			GameObjectAddCollider(gameObject, colliderObj);
		}
